...
public class FileSystemService {
    ...
    public void initUserNodes(String token) {
        Long faId = (Long) createNode("我的云盘", 0L, Constants.FsNodeType.FOLDER, token).getData();
        createNode("我的共享", faId, Constants.FsNodeType.FOLDER, token);
    }

    // 记得参数校验filename不能包含特殊字符
    @Transactional
    public Result<?> createNode(String fileName, Long faId, Integer fileType, String token) {
        Result<?> preResult = getAccess(faId, token, false);
    ...
    public Result<?> listFiles(Long faId, String token) {
        ...
        List<FileNodeVo> result = nodes.stream().map(node -> {
    ...
    private void copyBaseProperties(FsNode source, FileNodeVo target) {
    ...
    private Result<?> getAccess(Long nodeId, String token, Boolean strict) {
        User user = jwtUtil.getUserFromToken(token);
